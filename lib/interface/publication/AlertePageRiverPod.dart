...
class AlertePageRiverPod extends ConsumerStatefulWidget {
  final String? type;
  final String? userId;
  final bool? hideLabel;

  const AlertePageRiverPod({super.key, this.type, this.userId, this.hideLabel});

  @override
  ConsumerState<AlertePageRiverPod> createState() => _PublicationPageState();
}

class _PublicationPageState extends ConsumerState<AlertePageRiverPod> {
  ...
  void initState() {
    ...
    Future.microtask(
        () => ref.read(alerteNotifier.notifier).loadArticles(refresh: true));

    // Pagination
    _scrollController.addListener(() {
      final state = ref.read(alerteNotifier);
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          ...
          Expanded(
            child: state.loading && state.alerte.isEmpty
                ? const WaitWidget()
                : state.alerte.isEmpty
                    ? Container(
                        child: const Column(
                          children: [Text("Aucun element trouvé...")],
                        ),
                      )
                    : state.error != null && state.alerte.isEmpty
                        ...
                        : RefreshIndicator(
                            ...
                            child: ListView.builder(
                              ...
                              itemCount: state.alerte.length + 1,
                              itemBuilder: (context, index) {
                                if (index == state.alerte.length) {
                                ...
                                final Publication pub = state.alerte[index];
