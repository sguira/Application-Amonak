...
class UserService {
  ...
  static Future<http.Response> getAllUser(
    ...
    return await http
        .get(Uri.parse(server).replace(queryParameters: param),
            headers: authHeader)
        .then((value) {
  ...
  static Future<http.Response> getUser({required String userId}) async {
    return await http
        .get(Uri.parse("$apiLink/users/$userId"), headers: authHeader)
        .then((value) {
      return value;
    }).catchError((e) {
      return e;
  ...
  static Future<http.Response> searchUser(String query) async {
    return await http
        .get(Uri.parse(server).replace(queryParameters: {'search': query}),
            headers: authHeader)
        .then((value) {
      return value;
    }).catchError((e) {
  ...
  static Future sendFriend(String id) async {
    String resutl = '';
    Map data = {'to': DataController.user!.id, 'from': id};
    print(" id: $id");
    print("to ${DataController.user!.id}");
    await http.post(Uri.parse('$apiLink/friends/send'),
  ...
  static Future<http.Response> acceptRequestFriend(
    ...
    return await http
        .post(Uri.parse("$apiLink/friends/$api"),
            headers: authHeader,
            body: jsonEncode({"from": DataController.user!.id, "to": id}))
        .then((value) {
  ...
  static Future<http.Response> getBoutique({String? search}) async {
