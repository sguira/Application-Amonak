...
class _ButtonLikeState extends State<ButtonLike> {
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          }, icon: Icon( !isLiked ?Icons.favorite_border:Icons.favorite,color: isLiked?Colors.red:widget.color,)), 
          Text(NumberFormat.currency(locale: 'fr',decimalDigits: 0,symbol: '').format(likes),style: GoogleFonts.roboto(color:widget.color),)
        ],
      ),
    );
  }
  ...
  getNombreLike()async{
    await PublicationService.getNumberLike(widget.pub.id!, 'like').then((value){
