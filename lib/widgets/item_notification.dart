...
class ItemNotification extends StatefulWidget {
  ...
  const ItemNotification({super.key,required this.notification,required this.reloadNotificationData});
...
class _ItemNotificationState extends State<ItemNotification> {
  ...
  Widget build(BuildContext context) {
    return InkWell(
      onTap: (){
        ...
        if(widget.notification.action=="details_publication"&&widget.notification.idPub!=null){
          showModalBottomSheet(
            ...
            builder: (context)=>DetailsPublication(
              pubId: widget.notification.idPub!
            ) );
        }
        if(widget.notification.action=="share"&&widget.notification.idPub!=null){
          showModalBottomSheet(
            enableDrag: true,
            isScrollControlled: true,
                    ...
                    children: [
                      Container(
                        ...
                        child: InkWell(
                          onTap: (){
                            showDialog(
                              ...
                              return AlertDialog(
                                ...
                                title: Row(
                                  children: [
                                    Text("Suppression",style: GoogleFonts.roboto(color: Colors.black), ),
                                    const Spacer(),
                                ...
                                content: Container(
                                  child: Text("Voulez-vvous effacer cette notification ?",style: GoogleFonts.roboto(),),
                                ),
                                actions: [
                          ...
                          child: Icon(Icons.close,color: Colors.black.withAlpha(200),size:21)),
                      ),
                    ],
                  )
              ],
  ...
  deleteNotification(){
    NotificationService.deleteNotification(widget.notification.id!).then((value){
                              if(value.statusCode==200){
                                ...
                                errorSnackBar(message: 'Erreur lors de la suppression de la notification', context: context);
                              }
                            }).catchError((e){
                              errorSnackBar(message: 'Erreur lors de la suppression de la notification', context: context);
    });
